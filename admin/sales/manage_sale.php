...
<div class="content py-3">
    <div class="container-fluid">
        ...
        <img id="gcash-qr" src="C:\xampp\htdocs\cscs\qrcode.png" alt="Gcash QR Code" class="img-fluid d-none" style="max-width: 250px;">
    </div>
                                    </h3>
                                    <input type="number" id="payment_code" class="form-control form-control-sm rounded-0 d-none" name="payment_code" value="<?= isset($payment_code) ? $payment_code: "" ?>" placeholder="Reference No.">
...
<script>
    ...
    // Validate Tendered input
    var tendered = $('#tendered').val();
    if (tendered === '' || isNaN(tendered) || parseFloat(tendered) <= 0) {
        var el = $('<div>').addClass("alert alert-danger err-msg").text('Please enter a valid tendered amount.');
        _this.prepend(el);
        el.show('slow');
    ...
    // You can add another validation for 'Change' if required:
    // var change = $('#change').val();
    // if (change === '' || isNaN(change)) {
    //     var el = $('<div>').addClass("alert alert-danger err-msg").text('Please enter a valid change amount.');
    //     _this.prepend(el);
    //     el.show('slow');
    //     $("html, body, .modal").scrollTop(0);
    ...
    // If the validation passes, proceed to submit the form using AJAX
